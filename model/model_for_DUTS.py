...
class GET_Prototype(nn.Module):
    ...
    def forward(self, x, ss_map):
        ...
        for i in range(COM):
            ...
            ss_mask = ss_map_slice                                                      # ss_mask : [B, 1, H, W]

            prototype = self.Weighted_GAP(x, ss_mask)
            prototype = prototype.squeeze(-1).squeeze(-1).unsqueeze(1)                  # prototype : [B, 1, C]

            prototype_list.append(prototype)
        
        prototype_block = torch.cat(prototype_list, dim=1)                              # prototype_block : [B, com, C]
...
class Prototype_Sampler_Network(nn.Module):
    def __init__(self, in_channels):
        ...
        self.generate_adakernel = nn.Sequential(nn.Linear(channel * 3, channel),
    ...
    def forward(self, prototype_block):
        B, COM, C = prototype_block.size(0), prototype_block.size(1), prototype_block.size(2)
        ...
        ada_aux1 = graph_prototype

        graph_prototype = get_graph_feature(prototype_for_graph, k=10)
        graph_prototype = self.conv2(graph_prototype)
        graph_prototype = graph_prototype.max(dim=-1, keepdim=False)[0]
        ada_aux2 = graph_prototype

        graph_prototype = get_graph_feature(prototype_for_graph, k=10)
        graph_prototype = self.conv3(graph_prototype)
        graph_prototype = graph_prototype.max(dim=-1, keepdim=False)[0]
        ada_aux3 = graph_prototype

        ada_aux = torch.cat((ada_aux1, ada_aux2, ada_aux3), dim=2)
        ...
        adakernel = self.generate_adakernel(ada_aux)
        adakernel = adakernel.reshape(B, C, 3, 3)
        
        return total_samler_block, sampled_prototype_block, adakernel
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.ttotal_fusion6 = Res(channel, channel)
        self.ttotal_fusion5 = Res(channel, channel)
        self.ttotal_fusion4 = Res(channel, channel)
    ...
    def forward(self, image, rgb_ss_map):
        ...
        rgb_sampled_feature6 = self.rgb_GCM6(rgb_cf6, rgb_sampled_prototype_block)
        rgb_sampled_feature5 = self.rgb_GCM5(rgb_cf5, rgb_sampled_prototype_block)
        rgb_sampled_feature4 = self.rgb_GCM4(rgb_cf4, rgb_sampled_prototype_block)

        tortal_sampled_feature6 = rgb_sampled_feature6
        tortal_sampled_feature5 = rgb_sampled_feature5
        tortal_sampled_feature4 = rgb_sampled_feature4

        feat_56 = self.rgb_merge_co_56(torch.cat([tortal_sampled_feature5, resize(tortal_sampled_feature6, [22, 22])], dim=1))
        feat_45 = self.rgb_merge_co_45(torch.cat([tortal_sampled_feature4, resize(feat_56, [44, 44])], dim=1))
        ...
        return preds_list, rgb_sampler_block
